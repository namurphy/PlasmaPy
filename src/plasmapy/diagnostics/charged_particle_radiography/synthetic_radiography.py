...
class _SyntheticRadiographySaveRoutine(SaveOnceOnCompletion):
    ...
    def save(self) -> None:
        result_dictionary = self._particle_tracker.results_dict  # ty:ignore[possibly-missing-attribute]
...
class Tracker(ParticleTracker):
    ...
    def __init__(
        ...
        grids: AbstractGrid | Iterable[AbstractGrid],
            source: u.Quantity[u.m],  # ty:ignore[invalid-type-form]
            detector: u.Quantity[u.m],  # ty:ignore[invalid-type-form]
        dt=None,
        dt_range=None,
    ...
    _c = const.c.si.value  # ty:ignore[unresolved-attribute]
    ...
    def _apply_wire_mesh(
        ...
        # a wire
        h_centers = np.linspace(-width / 2, width / 2,
                                num=nwires[0])  # ty:ignore[not-subscriptable, unsupported-operator]
        for c in h_centers:
            hit |= np.isclose(xloc, c, atol=wire_radius)  # ty:ignore[no-matching-overload]

        v_centers = np.linspace(-height / 2, height / 2,
                                num=nwires[1])  # ty:ignore[not-subscriptable, unsupported-operator]
        for c in v_centers:
