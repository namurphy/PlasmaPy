...
c_si_unitless = const.c.si.value  # ty:ignore[unresolved-attribute]
e_si_unitless = const.e.si.value  # ty:ignore[unresolved-attribute]
m_p_si_unitless = const.m_p.si.value  # ty:ignore[unresolved-attribute]
m_e_si_unitless = const.m_e.si.value  # ty:ignore[unresolved-attribute]
...
def spectral_density(  # noqa: C901, PLR0912, PLR0915
        wavelengths: u.Quantity[u.nm],  # ty:ignore[invalid-type-form]
        probe_wavelength: u.Quantity[u.nm],  # ty:ignore[invalid-type-form]
    ...
    *,
    ...
    ifract=None,
    ions: ParticleLike = "p+",
        electron_vel: u.Quantity[u.m / u.s] = None,  # ty:ignore[invalid-type-form]
        ion_vel: u.Quantity[u.m / u.s] = None,  # ty:ignore[invalid-type-form]
    probe_vec=None,
    scatter_vec=None,
    instr_func: Callable | None = None,
        notch: u.m = None,  # ty:ignore[invalid-type-form]
) -> tuple[np.floating | np.ndarray, np.ndarray]:
    ...
    try:
        if sum(ion.charge_number <= 0 for ion in ions):  # ty:ignore[not-iterable, possibly-missing-attribute]
    ...
    alpha, Skw = spectral_density_lite(
        ...
        efract=efract,
        ifract=ifract,
        ion_z=ions.charge_number,  # ty:ignore[possibly-missing-attribute]
        ion_mass=ions.mass.to(u.kg).value,  # ty:ignore[possibly-missing-attribute]
