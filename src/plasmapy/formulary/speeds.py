...
def Alfven_speed(
    ...
    **Aliases:** `va_`
    ...
    if density.unit.physical_type == u.physical.mass_density:
        ...
        rho = (ion.mass + ion.charge_number * electron.mass) * density  # ty:ignore[possibly-missing-attribute]
...
def ion_sound_speed(
    ...
    ion: ParticleLike,
        n_e: u.Quantity[u.m ** -3] = None,  # ty:ignore[invalid-type-form]
        k: u.Quantity[u.m ** -1] = None,  # ty:ignore[invalid-type-form]
    gamma_e: float = 1,
    gamma_i: float = 3,
    Z=None,
) -> u.Quantity[u.m / u.s]:  # ty:ignore[invalid-type-form]
    ...
    **Aliases:** `cs_`
    ...
    try:
        V_S_squared = (
                      ...
                      ) / (ion.mass * (1 + klD2))  # ty:ignore[possibly-missing-attribute]
