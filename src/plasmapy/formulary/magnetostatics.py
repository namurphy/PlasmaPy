...
class MagnetoStatics(abc.ABC):
    ...
    @abc.abstractmethod
    def magnetic_field(self, p: u.Quantity[u.m]) -> u.Quantity[u.T]:  # ty:ignore[invalid-type-form]
        """
...
class MagneticDipole(MagnetoStatics):
    ...
    @validate_quantities
    def __init__(self, moment: u.Quantity[u.A * u.m ** 2], p0: u.Quantity[u.m]) -> None:  # ty:ignore[invalid-type-form]
    ...
    def magnetic_field(self, p: u.Quantity[u.m]) -> u.Quantity[u.T]:  # ty:ignore[invalid-type-form]
        ...
        m = self.moment
        B = (
                const.mu0.value  # ty:ignore[unresolved-attribute]
            / 4
            / np.pi
            * (
...
class GeneralWire(Wire):
    ...
    @validate_quantities
    def __init__(self, parametric_eq, t1, t2, current: u.Quantity[u.A]) -> None:  # ty:ignore[invalid-type-form]
    ...
    def magnetic_field(self, p: u.Quantity[u.m], n: int = 1000) -> u.Quantity[u.T]:  # ty:ignore[invalid-type-form]
        r"""
        Calculate magnetic field generated by this wire at position ``p``.
        ...
        B = B * const.mu0.value / 4 / np.pi * self.current  # ty:ignore[unresolved-attribute]
        return B * u.T
...
class FiniteStraightWire(Wire):
    ...
    def __init__(
            ...
            p1: u.Quantity[u.m],
            p2: u.Quantity[u.m],
            current: u.Quantity[u.A],  # ty:ignore[invalid-type-form]
    ) -> None:
        self.p1 = p1.value
        self.p2 = p2.value
        self._p1_u = p1.unit
    ...
    def magnetic_field(self, p) -> u.Quantity[u.T]:  # ty:ignore[invalid-type-form]
        r"""
        Calculate magnetic field generated by this wire at position ``p``.
...
class InfiniteStraightWire(Wire):
    ...
    def magnetic_field(self, p) -> u.Quantity[u.T]:  # ty:ignore[invalid-type-form]
        ...
        return B_unit / r * const.mu0.value / 2 / np.pi * self.current * u.T  # ty:ignore[unresolved-attribute]
...
class CircularWire(Wire):
    ...
    def __init__(
        self,
        normal,
            center: u.Quantity[u.m],  # ty:ignore[invalid-type-form]
            radius: u.Quantity[u.m],  # ty:ignore[invalid-type-form]
            current: u.Quantity[u.A],  # ty:ignore[invalid-type-form]
        n: int = 300,
    ) -> None:
    ...
    def magnetic_field(self, p) -> u.Quantity[u.T]:  # ty:ignore[invalid-type-form]
        ...
        t = x * np.pi
        pt = self.curve(t)  # ty:ignore[too-many-positional-arguments]
        dl = self.radius * (
